...
3. Le script écrit les **9 artefacts** `.sys/*` (recompte audit 2026-08-30 — `reverse_inventory.py` écrit aussi les 4 artefacts L0/L1) :
   ...
   `dependencies.json` et `config.json` sont des **pré-conditions de
   `/sdd-reverse-crosscut`** (FEATs transversales Librairies + Database).
...
├── code-graph.json                (L0)
├── data-access.json               (L1)
├── config.json                    (L1 — pré-condition /sdd-reverse-crosscut)
├── dependencies.json              (L1 — pré-condition /sdd-reverse-crosscut)
├── db-schema.{md,json}
