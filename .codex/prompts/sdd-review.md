...
Si `ArchReviewMode: full` → spawn agent `arch-reviewer` au STEP 3.0 ci-dessous
(batch fallback standalone — fix référence audit 2026-08-30, ex-« STEP 3.5 »
qui désigne la persistance interne de `sdd_review.py`).
...
Phase C (arch-reviewer) est câblée via le two-stage gate (cf. STEP 3.0.bis +
STEP 3.0 ci-dessus et `/dev-run` STEP 6.4). Tant que
