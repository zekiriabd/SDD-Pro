...
> les agents référencent par `@.sdd/rules/dev-shared-preflight.md`
...
```bash
python .sdd/python/sdd_scripts/preflight.py --family {family} --arg "{n}-{m}[:plan]"
```
...
`.sdd/python/sdd_scripts/preflight.py`.

---

## 2. STEP 0.5 — HARD-GATE context budget

Pattern partagé — appliquer `@.sdd/rules/build-and-loop.md §1`
...
Pattern partagé — appliquer `@.sdd/rules/build-and-loop.md §1.ter`
...
Pattern partagé — appliquer `@.sdd/rules/build-and-loop.md §1.bis`
