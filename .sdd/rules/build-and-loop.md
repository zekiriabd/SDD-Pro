...
> les Read `@.sdd/rules/{backend-first,dev-shared}.md` historiques
...
(`@.sdd/rules/build-and-loop.md`) plutôt que dupliqués. Évolution centrée ici.
...
```bash
python .sdd/python/sdd_scripts/context_budget.py \
  --agent {agent-id} \
...
Détail règle : `@.sdd/rules/ownership.md §1.bis`.
...
```bash
python .sdd/python/sdd_scripts/acquire_libname_lock.py \
...
3. Ne JAMAIS introduire une lib non déclarée dans `.sdd/stacks/{cat}/*.md`
...
`.sdd/rules/library-and-stack.md §3 (Partie A)` pour le format
...
```bash
python .sdd/python/sdd_scripts/mark_breaking_resolved.py \
...
- Aucune lib hors `.sdd/stacks/{cat}/*.md` actifs
...
- `@.sdd/rules/library-and-stack.md` (workflow Tech Lead + format
  ERROR HINT)
- `@.sdd/rules/ownership.md §1-§2, §4, §6.bis`
- `@.sdd/docs/principles/source-first.md` (si bug récurrent dans build_loop —
