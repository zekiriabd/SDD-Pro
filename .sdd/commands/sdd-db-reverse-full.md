...
4. **rung 1 — dispatch PAR VAGUES (le cœur de la refonte)** :
   ...
   b. **barrière de vague** : écrire les résumés produits dans
      `db-context.json.findings` (par l'orchestrateur, en une passe atomique —
      les agents n'écrivent que leur US, donc les écritures restent disjointes et
      le parallélisme reste sûr sans nouveau verrou), puis régénérer les packs
      de la vague suivante, qui citeront ces résumés au lieu de signaux bruts.
